package com.example.ideafoundation.demoapp;
...
/**
 * Created by ideafoundation on 10/01/17.
 */
...
public class MainActivity extends AppCompatActivity implements Spinner.OnItemSelectedListener {
    ...
    private String getName(int position){
        ...
        try {
            ...
            //Fetching name from that object
            name = json.getString(Config.TAG_RANK);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        //Returning the name
    ...
    private String getCourse(int position){
        ...
        try {
            JSONObject json = result.getJSONObject(position);
            course = json.getString(Config.TAG_POPULATION);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return course;
...
/*    //Doing the same with this method as we did with getName()
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        textViewCourse.setText(getCountry(position));
        textViewSession.setText(getCourse(position));
